...
namespace Nethermind.Synchronization
{
    ...
    public class SyncServer : ISyncServer
    {
        ...
        public Task BuildCHT()
        {
            return Task.Run(() =>
            {
                lock (_chtLock)
                {
                    ...
                    // todo - fix
                    if (maxSection == maxKnownSection) return;
                    for (long section = maxKnownSection; section <= maxSection; section++)
                    {
                        ...
                        {
                            var temp = sectionStart + blockOffset;
        ...
        public CanonicalHashTrie GetCHT(long sectionIndex)
        {
            if (sectionIndex > _cht.GetMaxSectionIndex()) 
                throw new EthSyncException($"Canonical hash trie does not yet contain section {sectionIndex}");

            return _cht.GetSubTrie(sectionIndex);
        }
